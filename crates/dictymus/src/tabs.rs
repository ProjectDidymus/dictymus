...
pub struct DictionaryTab {
	...
	pub article_html: Rc<RefCell<String>>,
}
...
impl TabManager {
	pub fn new(
		...
		Self { notebook, tabs: Vec::new(), base_font, status_bar }
	}

	pub fn build_tab_panel(&self, dict: Rc<DictHandle>, frame: Frame) -> Rc<DictionaryTab> {
		...
		let article_sizer =
			StaticBoxSizerBuilder::new_with_label(Orientation::Vertical, &article_panel, "Article")
				.build();
		let article_box =
			article_sizer.get_static_box().expect("StaticBoxSizer must have a StaticBox");
		let article =
		...
		let article_html = Rc::new(RefCell::new(String::new()));
		let rc = Rc::new(DictionaryTab {
			...
			article_html: Rc::clone(&article_html),
		});
		rc.article.register_handler("assets", move |uri| {
			...
			if uri.ends_with("/SBL_BLit.ttf") {
				...
				Some(WebViewHandlerResponse {
					data: article_html.borrow().as_bytes().to_vec(),
